# denissepareja
Data Scientist &amp; Healthcare Data Analyst | Python, SQL, Dashboards &amp; Analytics Portfolio

# Hi! I’m Denisse Pareja 👋

### Data Scientist & Healthcare Data Analyst

I’m a Data Scientist with a strong background in healthcare and global health operations, focused on transforming complex data into actionable insights that drive better decisions and measurable impact.

---

## 🔍 About Me

- 🎯 I am a Data Scientist with a strong background in healthcare, focused on transforming complex data into actionable insights that drive better decisions and measurable impact.
- 📊 Passionate about using data to improve healthcare operations and outcomes  
- 🧠 Strong background in research, analytics, and structured problem-solving  
- 💻 Experienced with Python, SQL, and data visualization tools  
- 🌎 Fluent in English and Spanish  

---

## 🛠 Technical Skills

**Programming & Analytics**
- Python (Pandas, NumPy, Scikit-learn)
- SQL
- Exploratory Data Analysis (EDA)
- Machine Learning basics
- Data Cleaning & Preprocessing
- Statistics & Hypothesis Testing
- ETL Processes

**Data Visualization**
- Tableau  
- Power BI  
- Matplotlib / Seaborn  
- KPI Dashboards
- Streamlit  

**Tools & Platforms**
- Git & GitHub  
- Jupyter Notebooks  
- Excel  
- VS Code  

---

## 📂 Featured Projects

### 📌 Claims Analytics and Cost Prediction
**Technologies:** Python, Pandas, Scikit-learn  
- Built an end-to-end machine learning pipeline to predict healthcare costs  
- Performed feature engineering and regression modeling  
- Achieved an R² of 0.86 using Random Forest  
- Identified smoking status, BMI, and age as key cost drivers  

🔗 Repository: https://github.com/denpareja/insurance-claims-analytics

---

### 📌 Employee Churn Prediction – People Analytics
**Technologies:** Python, Machine Learning, Data Preprocessing  
- Developed a classification model to predict employee attrition  
- Conducted EDA and feature engineering  
- Optimized model using AUC-ROC metrics  
- Delivered actionable HR insights  

🔗 Repository: https://github.com/denpareja/people-analytics-employee-churn

---

### 📌 McDonald's Nutrition Analytics App
**Technologies:** Python, Streamlit  
- Built an interactive app to analyze nutritional data  
- Implemented data filtering and visualization  
- Enabled comparison between menu items  

🔗 Repository: https://github.com/denpareja/mcdonalds-nutrition-app

---

## 🎓 Education

**TripleTen – Data Science Bootcamp**  
Hands-on training in Python, Machine Learning, and Data Analytics

**Master of Science in Public Health**  
University of Miami

**Doctor of Medicine (MD)**  
Universidad Católica de Santiago de Guayaquil

---

## 📊 Areas of Interest

- Healthcare Analytics
- Predictive Modeling
- Business Intelligence
- Operations Optimization
- Data-Driven Decision Making

---

## 📫 Let’s Connect!

- LinkedIn: https://www.linkedin.com/in/denissepareja  
- GitHub: https://github.com/denpareja  
- Email: dparejaval@gmail.com  

---

Thanks for visiting my profile! Feel free to explore my projects and reach out for collaboration or opportunities 🚀
